                                ...
                                <tbody>
                                    @foreach ($requests as $key => $val)
                                        <tr>
                                            ...
                                            <td>{{ @$val->message }}</td>
                                            <td><a href="{{ asset($val->image) }}" target="_blank"><img
                                                        src="{{ asset($val->image) }}" width="180px" /></a></td>
                                            <td>
                                                @if ($val->status == 0)
                                                    <span class="text-green"><b>Solved</b></span>
                                                @elseif($val->status == 1)
                                                    <span class="text-red"><b>Pending</b></span>
                                                @elseif($val->status == 2)
                                                    <span class="text-orange"><b>Hold</b></span>
                                                @endif
                                            </td>
                                            <td>{{ $val->created_at }}</td>
                                            <td>
                                                @if ($val->status == 1 || $val->status == 2)
                                                    <a href="{{ route('complaint-action', $val->id) }}"
                                                        onclick="return confirm('Are you sure want to perform this action?')"
                                                        class="btn btn-success mb-2 ">Solve</a>
                                                @endif
                                            </td>
                                        </tr>
                                    @endforeach
